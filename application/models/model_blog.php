...
class Model_blog extends CI_Model {
	...
	public function get_ordena_por_id($limit=0,$start=0,$paginacion=false){
		if ($paginacion) {
			$this->db->limit($limit,$start);
			$this->db->order_by('id', 'asc');
		}
		$productos1 = $this->db->get('blog');
	...
	public function get_ordena_por_autor($limit=0,$start=0,$paginacion=false){
		if ($paginacion) {
			$this->db->limit($limit,$start);
	...
	public function get_ordena_por_categoria($limit=0,$start=0,$paginacion=false){
		if ($paginacion) {
			$this->db->limit($limit,$start);
	...
	public function get_ordena_por_inicio($limit=0,$start=0,$paginacion=false){
		if ($paginacion) {
			$this->db->limit($limit,$start);
			$this->db->order_by('pag_inicio', 'asc');
		}
		$productos1 = $this->db->get('blog');
	...
	public function get_ordena_por_estado($limit=0,$start=0,$paginacion=false){
		if ($paginacion) {
			$this->db->limit($limit,$start);
	...
	public function get_post($limit=0,$start=0,$paginacion=false){
		if ($paginacion) {
			$this->db->limit($limit,$start);
	...
	public function get_post_by_slug($value=''){
		...
		$query = $this->db->get_where('blog', array('slug' => $value ));
		if($query->num_rows > 0){
			// Si existe
			return FALSE;
		}else{
	...
	public function checaVisitas($id = 0){
    	$this->db->where('id', $id);
    	$this->db->select('num_visitas');
    	$query = $this->db->get('post');
    	$num = $query->row('num_visitas');
    	$this->db->where('id', $id);
    	$this->db->update('blog', array('num_visitas' => $num+1));
    }
	...
	public function dameArticulosMasLeidos(){
		$query = $this->db->query("select * from (select t.id_pagina, t.cantidad from (select id_pagina, count(id_pagina) as cantidad from log_articulos group by id_pagina) as t order by cantidad desc) as t left join blog as p on p.id = t.id_pagina LIMIT 0, 4 ");
		if($query->num_rows() > 0){
			return $query->result();
		}else{
			return false;
	...
	public function blog($limit = false, $start = false){
		...
		$this->db->select('blog.*, usuarios.nombre as autor, categoria.nombre as categoria_nombre');
		$this->db->from('blog');
		$this->db->join('categoria', 'categoria.id = blog.id_categoria');
		$this->db->join('usuarios','usuarios.id = blog.id_autor', 'left');
		$this->db->where(array('blog.tipo' => 1, 'blog.estado' => 1));
		$query = $this->db->get();
		if($query->num_rows() > 0){
			return $query->result();
		}else{
	...
	public function dameSlugCategoria($id){
		$this->db->where('id',$id);
		$this->db->limit(1);
		$query = $this->db->get('categoria');
		if($query->num_rows() > 0){
	...
	public function damePostDestacados(){
		$this->db->select('blog.*, categoria.slug as categoria_slug');
		$this->db->from('blog');
		$this->db->join('categoria', 'categoria.id = blog.id_categoria');
		$this->db->where(array('estado' => 1, 'tipo' => 1, 'pag_inicio' => 1));
		$query = $this->db->get();
	...
	public function dameMasRecientes(){
		$this->db->where(array('tipo' => 1, 'estado' => 1));
		$this->db->order_by('id', 'desc');
